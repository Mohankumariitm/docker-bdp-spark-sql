...
echo_latest_version() {
  ...
  version="$(curl -kfsSLI -o /dev/null -w "%{url_effective}" https://github.com/cdr/code-server/releases/latest)"
...
main() {
  ...
  if [ "${RSH_ARGS-}" ]; then
    ...
    curl -kfsSL https://code-server.dev/install.sh | prefix "$RSH_ARGS" "$RSH" "$RSH_ARGS" sh -s -- "$ALL_FLAGS"
    return
  fi
...
install_aur() {
  ...
  sh_c "curl -#fksSL https://aur.archlinux.org/cgit/aur.git/snapshot/code-server.tar.gz | tar -xzC $CACHE_DIR/code-server-aur --strip-components 1"
